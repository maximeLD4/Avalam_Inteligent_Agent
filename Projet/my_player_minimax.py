...
class MyAgent(Agent):
    ...
    def play(self, percepts, player, step, time_left):
        ...
        def minimax_search(board, profondeur=DEFAULT_PROF):
            if player > 0:
                v, m = max_val(board, profondeur)
            elif player < 0:
                v, m = min_val(board, profondeur)
            return v, m

        def max_val(board, profondeur):
        ...
        def min_val(board, profondeur):
            ...
            if profondeur == 0:
                ...
                for action in actions:
                    score_intermediaire0 = board.clone()
                    score_intermediaire0.play_action(action)
                    score_intermediaire1, an_action = max_val(score_intermediaire0, profondeur - 1)
                    if score_intermediaire1 < score_best_action:
                        score_best_action = score_intermediaire1
                        best_action = action
                return score_best_action, best_action
