        ...
        def utility_2(board):
            ...
            for row in range(0, rows, 1):
                for column in range(0, columns, 1):
                    # Partie d'Heuristique qui permet de compter le score actuel
                            ...
                            for row_conv_1 in range(-1, 2, 1):
                                for column_conv_1 in range(-1, 2, 1):
                                    if row + row_conv_1 < 8 and row + row_conv_1 > 0 and column + column_conv_1 < 8 and column + column_conv_1 > 0:
                                        ...
                                        if (current_tower_score_check == board.max_height - score_incomplet):
                                            positives_tower_score += 10
                                            negatives_tower_score += 10
                                        elif (current_tower_score_check == -board.max_height + score_incomplet):
                                            negatives_tower_score -= 10
                                            positives_tower_score -= 10
                            ...
                            if compteur_de_0_filtre3 == 4+2*tailles_groupes[i][0]+2*tailles_groupes[i][1]:
                                ...
                                for column_group in range(0, tailles_groupes[i][1], 1):
                                    for row_group in range(0, tailles_groupes[i][0], 1):
                                        ...
                                        if element_groupe !=0 and abs(element_groupe) != board.max_height:
            ...
            if profondeur == 0:
                ...
                for action in actions:
                    ...
                    if (score_best_action <= alpha):
                        return score_best_action, best_action
                return score_best_action, best_action
        action_opening_positive = [[8,6,7,6],[8,6,7,6],[6,6,6,7],[6,6,6,7],[7,6,7,7],[7,6,7,7],[6,7,7,7],[6,7,7,7],[9,9,9,9],[9,9,9,9],[9,9,9,9],[9,9,9,9],[9,9,9,9],[9,9,9,9],[9,9,9,9]]
        action_opening_negative = [[7,6,8,6],[7,6,8,6],[6,7,6,6],[6,7,6,6],[8,6,7,7],[8,6,7,7],[6,6,7,7],[6,6,7,7],[9,9,9,9],[9,9,9,9],[9,9,9,9],[9,9,9,9],[9,9,9,9],[9,9,9,9],[9,9,9,9]]
        #action_opening_negative = [[8,6,7,6],[6,6,6,7],[7,6,7,7],[6,7,7,7]]
        #action_opening_positive = [[7,6,8,6],[6,7,6,6],[8,6,7,7],[6,6,7,7]]
        n_act = step - 1 #int((step-1)/2)
        print("step = ", step)
        if player > 0:
            print("step : ", step, " action : ", action_opening_positive[n_act], "valide ? : ", board.is_action_valid(action_opening_positive[n_act]))
            if step <= 8:
                if board.is_action_valid(action_opening_positive[n_act]):
                    print("open")
                    return action_opening_positive[n_act]
            else:
                score_best_action, best_action = minimax_search(board, 2)  # On regarde deux coup en avant
                return best_action
        elif player < 0:
            print("step : ", step, " action : ", action_opening_positive[n_act])
            if step <= 8:
                if board.is_action_valid(action_opening_negative[n_act]):
                    print("open")
                    return action_opening_negative[n_act]
            else:
                score_best_action, best_action = minimax_search(board, 2)  # On regarde deux coup en avant
                return best_action
