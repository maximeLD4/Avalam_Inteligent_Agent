...
class MyAgent(Agent):
    ...
    def play(self, percepts, player, step, time_left):
        ...
        def time_limit(step_arg):
            if step_arg <= STEP_CHANGE:
                return CST_MULT_1 * (CST_ADD + CST_MULT_2 * (1 / (2 * (np.sqrt(SIGMA * np.pi)))) * (
                    np.exp((-1 / 2) * (step_arg - MOY) * (step_arg - MOY) / (SIGMA * SIGMA))))
            else:
                return CST_MULT_1 * 31
                                ...
                                for column_conv_1 in range(-1, 2, 1):
                                    if 8 > (row + row_conv_1) > 0 and 8 > (column + column_conv_1) > 0:
                                        ...
                                        if current_tower_score_check == -player * (
                                            ...
                                            break
                    # Partie de l'heuristique qui permet de maximise les tour ou on est en infériorité, on pénalise légèrement les tour ou on est pas majoritaire
            return score_tot
            ...
            if not actions:
                ...
                for action in actions:
                    ...
                    if player * utility(board_clone) >= THRESHOLD:
                        ...
                        if time.time() - start_t >= time_left_to_play and depth != 0 and best_action_max is not None: #and best_action_max is not None:  # AJOUT NEW
                            return score_best_action_max, best_action_max  # AJOUT NEW
